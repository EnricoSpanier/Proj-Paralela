...
int main(int argc, char* argv[]) {
    ...
    // Fase 1: Construir tabela hash (sequencial, pois é inicialização)
    ...
    // Fase 2: Pré-carregar linhas do log (leitura sequencial)
    ...
    // Fase 2b: Processar em paralelo; restringe critical somente ao incremento
    #pragma omp parallel for schedule(static)
    for (long i = 0; i < (long)lines_count; i++) {
        char* url = extract_url(lines[i]);
        if (url) {
            CacheNode* node = ht_get(ht, url); /* leitura fora da região crítica */
            if (node) {
                #pragma omp critical
                {
                    node->hit_count++;
